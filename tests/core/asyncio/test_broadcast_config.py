...
async def test_broadcasts_to_all_endpoints(server_with_two_clients):
    ...
    await client_a.subscribe(BroadcastEvent, return_queue.put_nowait)
    await client_b.subscribe(BroadcastEvent, return_queue.put_nowait)
...
async def test_broadcasts_to_specific_endpoint(server_with_two_clients):
    ...
    await client_a.subscribe(BroadcastEvent, return_queue.put_nowait)
    await client_b.subscribe(BroadcastEvent, return_queue.put_nowait)
    await client_a.subscribe(TailEvent, return_queue.put_nowait)
    await client_b.subscribe(TailEvent, return_queue.put_nowait)
